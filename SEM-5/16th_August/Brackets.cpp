...
// Mike is willing to replace A with B if F(A) is equal to F(B). He would also like to choose B
// with the minimal possible length amongst ones satisfying the previous condition. If there are several
// such strings with the minimal possible length, then Mike will choose the least one lexicographically,
...
// The only line of each testcase contains one string A denoting Mike's parentheses sequence. It is guaranteed
// that A only consists of the characters '(' and ')'. It is also guaranteed that A is a valid parentheses sequence.

// Output
// The output should contain exactly T lines, one line per each testcase in the order of their appearance.
// The only line of each testcase should contain one string B denoting the valid parentheses sequence that
// should be chosen by Mike to replace A.
