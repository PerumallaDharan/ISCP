...
// restaurant whose total price is exactly p. The current menus of Ciel's restaurant are shown the
// following table.
...
// So please find the minimum number of menus whose total price is exactly p. Note that if she orders
// the same menu twice, then it is considered as two menus are ordered. (See Explanations for details)

// Input
// The first line contains an integer T, the number of test cases. Then T test cases follow. Each test
// case contains an integer p.
...
// In the last sample, the optimal way is 2048+2048=4096 (2 menus). Note that there is no menu whose
// price is 4096.
